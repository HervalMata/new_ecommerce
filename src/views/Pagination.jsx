...
const Pagination = ({ pageNumber, setPageNumber, totalItem, perPage, showItem }) => {
    ...
    const createButton = () => {
        ...
        for (let i = startPage; i <= endPage; i++) {
            btns.push(
                <li className={`${pageNumber === i ? 'bg-indigo-300 shadow-lg shadow-indigo-300/50 text-white' 
                         : 'bg-slate-600 hover:bg-indigo-400 shadow-lg hover:shadow-indigo-500/50 ' +
                    'hover:text-white text-[#D0D2D6]'} w-[33px] h-[33px] rounded-full flex items-center 
